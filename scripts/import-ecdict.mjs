...
const booksOutputDir = path.join(root, "src", "data", "books");
const metaOutputPath = path.join(root, "src", "data", "bookMeta.js");
...
ensureDir(booksOutputDir);
const bookMeta = wordBooks.map(({ words, ...book }) => ({ ...book, totalCount: words.length }));
fs.writeFileSync(
  metaOutputPath,
  `// Generated by scripts/import-ecdict.mjs from ECDICT (${sourceUrl}).\n` +
    `// Do not edit individual entries manually; update the import script instead.\n\n` +
    `export const bookMeta = ${JSON.stringify(bookMeta, null, 2)};\n`,
  "utf8",
);
for (const book of wordBooks) {
  fs.writeFileSync(
    path.join(booksOutputDir, `${book.id}.js`),
    `// Generated by scripts/import-ecdict.mjs from ECDICT (${sourceUrl}).\n` +
      `// Do not edit individual entries manually; update the import script instead.\n\n` +
